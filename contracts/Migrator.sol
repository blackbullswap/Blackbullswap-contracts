...
// File contracts/blackbull-periphery/interfaces/IBridgeToken.sol

pragma solidity >=0.5.0;

interface IBridgeToken is IBlackbullERC20 {
    function swap(address token, uint256 amount) external;
    function swapSupply(address token) external view returns (uint256);
}


// File contracts/blackbull-periphery/libraries/Roles.sol
...
// File contracts/blackbull-periphery/BlackbullBridgeMigrationRouter.sol
...
contract BlackbullBridgeMigrationRouter {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'BlackbullBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'BlackbullBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "BlackbullBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "BlackbullBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "BlackbullBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "BlackbullBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IBlackbullERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IBlackbullPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = BlackbullLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = BlackbullLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IBlackbullPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IBlackbullPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "BlackbullBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "BlackbullBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "BlackbullBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IBlackbullPair(pairA).token0() == IBlackbullPair(pairB).token0() ||
            IBlackbullPair(pairA).token0() == IBlackbullPair(pairB).token1() ||
            IBlackbullPair(pairA).token1() == IBlackbullPair(pairB).token0() ||
            IBlackbullPair(pairA).token1() == IBlackbullPair(pairB).token1(),
            "BlackbullBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "BlackbullBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "BlackbullBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IBlackbullPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IBlackbullPair(liquidityPairFrom).token0();
        if (
            IBlackbullPair(liquidityPairFrom).token0() == IBlackbullPair(liquidityPairTo).token0() ||
            IBlackbullPair(liquidityPairFrom).token0() == IBlackbullPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IBlackbullPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "BlackbullBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IBlackbullPair(liquidityPairTo).token0() ||
            newTokenAddress == IBlackbullPair(liquidityPairTo).token1(),
            "BlackbullBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IBlackbullPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IBlackbullPair(liquidityPairFrom).token0() != IBlackbullPair(liquidityPairTo).token0() &&
            IBlackbullPair(liquidityPairFrom).token1() != IBlackbullPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IBlackbullPair(liquidityPairTo).token0(), IBlackbullPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IBlackbullPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IBlackbullPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IBlackbullPair(pairAddress).getReserves();
        uint totalSupply = IBlackbullPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "BlackbullBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "BlackbullBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IBlackbullPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = BlackbullLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
